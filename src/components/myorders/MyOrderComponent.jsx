	...
	render() {
		...
		const mydata = this.state.cart.map((cart) => {
			return (
				<div
					class="card"
					style={{ marginLeft: "50px", marginRight: "10px", marginTop: "20px" }}
				>
					<img
						src={`http://localhost:4000/uploads/${cart.image}`}
						alt="Denim Jeans"
					...
					<p>
						<button onClick={() => this.deletecart(cart._id)}>
							Cancel Order
						</button>
					</p>
				</div>
		...
		return (
			<div>
				...
				<div className="container-fluid">
					...
					<div className="row">
						<div className="col-sm-12">{mydata}</div>
					</div>
				</div>
			</div>
		);
